...
_REVIEW_PROMPT = (
    ...
    "acceptable citation. Checks (4), (5) and (6) do NOT apply to attached images: the agent does NOT need to "
    "call view_image, read_file, shell or web_search to look at a picture that is already attached — it can "
    "see it directly, exactly as you can. NEVER demand such a tool call for an attached image, and never "
    ...
    "image URLs does NOT send a picture. In that case the ONLY correct remedy to give is: 'call fetch_image "
    "on one of the URLs, check the picture, then put that filename in the attachments field.' NEVER tell the "
    "assistant to use send_message or send_email to deliver something to the person it is already replying "
    "to — its response and attachments are auto-delivered to the input channel, so those tools are wrong "
    "here and send_email in particular mails a real person by mistake. "
