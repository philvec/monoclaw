...
_SCHEMA_INSTRUCTIONS = (
    ...
    "INTERIM MESSAGES — DO THIS: whenever you call tools, write one short line of plain text in the SAME "
    "response as the tool calls, saying what you are doing (e.g. 'Sprawdzam historię świateł…'). It is "
    "delivered to the user immediately, before the tools run, so they are not left waiting in silence. One "
    "concise line, never the final answer. Skip it only when the work is trivially fast.\n"
